  ...
  if (
    ...
    if (symbol && exchange) {
      ...
      const matchingPosition = positions.find(
        pos => pos.symbol === symbol && pos.exchange === exchange
      )

      if (matchingPosition) {
        if (quantity > matchingPosition.quantity) {
          errors.push(
            `Insufficient position. Available: ${matchingPosition.quantity.toFixed(6)}`
          )
        }
      } else {
        // No position exists for this symbol/exchange
        errors.push('No open position for this instrument')
      }
    }
  }
